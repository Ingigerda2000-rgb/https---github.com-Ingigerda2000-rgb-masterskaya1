...
try:
    ...
    print(f"   Всего изделий в базе: {all_products.count()}")
    ...
    print(f"   ❌ Ошибка при получении изделий: {e}")

# 2. Проверяем активные изделия
print("\n2. АКТИВНЫЕ ТОВАРЫ:")
try:
    active_products = Product.objects.filter(status='active')
    print(f"   Активных изделий: {active_products.count()}")
    
    if active_products.exists():
        print("   Список активных изделий:")
        for p in active_products:
            print(f"     - {p.name} (ID: {p.id})")
    else:
        print("   ❌ НЕТ активных изделий!")
        print("   Проверьте статусы изделий в админке")
...
try:
    if active_products.exists():
        ...
        if main_image:
            ...
            print(f"   ⚠ У изделия нет основного изображения")
            
        # Проверяем изображения
        images_count = product.images.count()
        print(f"   Изображений у изделия: {images_count}")
    else:
        print("   ⚠ Нет активных изделий для проверки")
...
if active_products.exists():
    print("✅ Есть активные изделия в базе")
    print("⚠ Проблема в отображении (шаблон или контекст)")
else:
    print("❌ НЕТ активных изделий!")
    print("👉 Перейдите в админку и измените статус изделий на 'active'")
